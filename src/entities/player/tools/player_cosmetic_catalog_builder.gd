...
const ASSETS_ROOT: String = "res://src/entities/player/assets"
const LEGACY_VARIANT_RELATIVE_PATH: String = "32x32/normal_body_idle.png"
...
func build_catalog() -> Dictionary:
	...
	_merge_summary(result, _rebuild_slot(catalog, "head_variants", "head_id", ASSETS_ROOT, "head"))
	_merge_summary(result, _rebuild_slot(catalog, "body_variants", "body_id", ASSETS_ROOT, "body"))
	_merge_summary(result, _rebuild_slot(catalog, "legs_variants", "legs_id", ASSETS_ROOT, "legs"))
...
func _find_slot_sprite_paths(slot_root: String) -> PackedStringArray:
	...
	while true:
		...
		if item.is_empty():
			break
		if item.begins_with(".") or not dir.current_is_dir():
			continue
		var sprite_path: String = slot_root.path_join("%s/%s" % [item, LEGACY_VARIANT_RELATIVE_PATH])
